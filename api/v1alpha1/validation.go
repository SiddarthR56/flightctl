...
func validateApplications(apps []ApplicationProviderSpec, fleetTemplate bool) []error {
	...
	for _, app := range apps {
		...
		case ImageApplicationProviderType:
			...
			if provider.Image == "" {
				allErrs = append(allErrs, fmt.Errorf("image application provider: image is required"))
				continue
			}
...
func ensureAppName(app ApplicationProviderSpec, appType ApplicationProviderType) (string, error) {
	...
	case ImageApplicationProviderType:
		...
		if app.Name == nil {
			if provider.Image == "" {
				return "", fmt.Errorf("image is required when application name is not provided")
			}
			app.Name = &provider.Image
		}
